...
| 기능 | 설명 |
|---|---|
| 실물 검색 | 색 스와치 + 모양 칩(**색·모양 모두 여러 개 선택** — 타원형/장방형처럼 분류가 애매해도 한 번에) + 앞면 각인으로 거꾸로 찾기(기본 탭). 각인은 **실제 인쇄 각인**만(마크는 별도). **거꾸로 집은 알약 보정**: 각인을 180° 뒤집어도 매칭(예: 각인 `SIH`를 `HIS`로 검색해도 일치) |
...
| 결과 카드 | 품목명·업체·색/모양/각인(앞·뒤, 검색어 강조)·분할선·제형. 각인이 180° 뒤집어 일치한 경우 **`↕ 거꾸로 보면 일치`** 배지. **약 글리프 탭 → 확대**(실사진 원본 링크) |
