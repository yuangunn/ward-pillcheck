...
| 마크로 찾기(그려서) | 캔버스에 **마크를 그리면** 번들 마크와 **온디바이스 형상 유사도**로 닮은 후보 제시(서버·API 0). 그린 게 **글자/숫자면 OCR로 인식**(베타·Tesseract 동적 로드) → 마크 글자 검색에 연결(예: G 그리기→«ㄷㄱ,G» 찾기). *OCR 모델은 첫 사용 시 인터넷 필요* |
| 이름 검색 | 품목명으로 검색 |
