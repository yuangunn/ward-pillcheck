...
| 기능 | 설명 |
|---|---|
| 실물 검색 | 색 스와치 + 모양 칩(**색·모양 모두 여러 개 선택** — 타원형/장방형처럼 분류가 애매해도 한 번에) + 앞면 각인으로 거꾸로 찾기(기본 탭). **비슷한 계열 색 한 번에**: 헷갈리기 쉬운 색은 계열로 묶어, 한 색을 처음 누르면 계열 전체가 함께 선택(분홍→주황·빨강, 연두↔초록↔청록, 파랑↔남색, 자주↔보라) — **켜기는 계열 통째로, 끄기는 개별**로 미세조정. 각인은 **실제 인쇄 각인**만(마크는 별도). **거꾸로 집은 알약 보정**: 각인을 180° 뒤집어도 매칭(예: 각인 `SIH`를 `HIS`로 검색해도 일치). **혼동문자 보정**(3글자+): 마모·오독으로 헷갈리는 글자(`0↔O↔D · 1↔I↔L · 5↔S · 8↔B · 2↔Z · 6↔G`)도 매칭(예: `0LT`로 `DLT` 검색) — 정확 일치를 앞에 두고 그 뒤에 표시 |
...
| 인계(원내 Teams 전용) | 복약 리스트를 한 줄 포맷 텍스트로 **원내 Teams**로만 전송(병원 도메인/.ac.kr 계정만 허용). 외부 복사·시스템 공유는 제공 안 함. **공유 상세설정**에서 **성분명(한글 괄호)·영문 품목명·겉모습** 표시를 각각 토글(기기에 저장). 영문 품목명은 허가정보 **품목영문명(ITEM_ENG_NAME)**을 이름 뒤 대괄호로 병기(예: `모티리톤정 [Motilitone Tab.]`) — 직접입력·영문명 미등록 약은 생략 |
...
- **실물사진**(선택): 본 사진은 SW(`pill-photos`, CacheFirst)로 자동 보관. 설정에서 **전부 미리 받기**(대용량 ~수 GB, 중단 가능)도 가능 — 식약처에서 폰으로 직접(레포/Pages 호스팅 없음).
  - **콜드 로딩**: 처음 보는 약 사진은 nedrug→Worker 경유가 느려(콜드 ~20초, 캐시 후 <1초) 그동안 상세 시트에 **로딩 스피너** 표시(사진 '없음'이 아니라 '받는 중'). 한 번 보면 엣지 캐시(7일)로 즉시.
