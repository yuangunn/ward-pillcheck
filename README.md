...
| 인계(원내 Teams 전용) | 복약 리스트를 한 줄 포맷 텍스트로 **원내 Teams**로만 전송(병원 도메인/.ac.kr 계정만 허용). 외부 복사·시스템 공유는 제공 안 함. **공유 상세설정**에서 **성분명(한글 괄호)·영문 성분명(INN 병기)·겉모습** 표시를 각각 토글(기기에 저장). 영문 성분명은 상세화면과 같은 **영문 INN**을 이름 뒤 대괄호로 병기(예: `트라젠타정 [Linagliptin]`) — 직접입력·번들 미보유 약은 생략 |
