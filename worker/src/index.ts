...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/drugsearch') {
      ...
      if (inj) {
        // 외용약·주사제(낱알 모양 없는 비경구 약)만 남긴다.
        // 엄격 적용: 경구 액/시럽/pack 약(라미나지액·시네추라시럽·볼그레액 등)은
        // 이 키워드에 안 걸리므로 외용·주사제 탭에서 제외한다(빈 결과여도 원본으로 되돌리지 않음).
        // '펜'은 펜주사(…펜주/끝의 펜)만, '겔'은 끝(…겔)·겔제만 — 부루펜·겔포스 등 경구약 오검출 방지.
        const re = /(주사|주입|펜(?=주|$)|카트리지|바이알|키트|프리필드|주사액|주사제|인슐린|플렉스|퀵펜|주\)|주$|흡입|에보할러|할러|디스커스|레스피맷|터부할러|네뷸|점안|점이|점비|좌제|좌약|질정|질좌|연고|크림|로션|겔(?=$|제)|젤|패치|첩부|스프레이|분무|에어로|도포|외용|카타리)/;
        items = items.filter((x: { itemName: string }) => re.test(x.itemName));
      }
