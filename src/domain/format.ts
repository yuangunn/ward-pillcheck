...
/**
 * 영문 품목명 한 줄 표기 정리: 연속 공백 → 1칸, 양끝 트림.
 * (허가정보 ITEM_ENG_NAME 원문을 쓰되, 한 줄 인계에 붙기 좋게만 다듬는다.)
 * 예) 'Motilitone  Tab.' → 'Motilitone Tab.'
 */
...
/** 인계 공유 옵션: 성분명·겉모습·영문 품목명 포함 여부, 환자 라벨 가리기 */
export interface ShareOpts {
  ingredient?: boolean; // 품목명의 성분 괄호(…) 포함
  appearance?: boolean; // 겉모습(색/모양/각인) 포함
  english?: boolean; // 영문 품목명 병기 — 예) 모티리톤정 [Motilitone Tab.]
  englishBySeq?: Map<string, string>; // itemSeq → 영문 품목명(허가정보 ITEM_ENG_NAME). 공급: 번들/데모에서 조회
  mask?: boolean; // 환자 라벨 가운데 가리기. 미지정/true=가림(기본), false=원문 노출
}

/** 공유용 약 한 줄: 이름(+옵션에 따라 영문 품목명)/용량(+옵션에 따라 겉모습) + 메모.
...
function shareMedLine(med: MedItem, opts: ShareOpts): string {
  ...
  // 영문 품목명은 itemSeq 로 매핑된 값이 있을 때만 대괄호로 병기(직접입력·영문명 미등록 약은 생략).
